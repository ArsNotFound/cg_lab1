...
void LightingTechnique::setPointLights(const std::vector<PointLight> &lights) const {
    ...
    for (auto i = 0; i < lights.size(); i++) {
        ...
        glUniform3f(
            mPointLightsLocation[i].position,
            lights[i].position.x,
            lights[i].position.y,
            lights[i].position.z
        );
        glUniform1f(
            mPointLightsLocation[i].attenuation.constant, lights[i].attenuation.constant
        );
        glUniform1f(
            mPointLightsLocation[i].attenuation.linear, lights[i].attenuation.linear
        );
...
void LightingTechnique::setSpotLights(const std::vector<SpotLight> &lights) const {
    ...
    for (auto i = 0; i < lights.size(); i++) {
        ...
        glUniform3f(
            mSpotLightsLocation[i].position,
            lights[i].position.x,
            lights[i].position.y,
            lights[i].position.z
        );
        glUniform1f(
            mSpotLightsLocation[i].attenuation.constant, lights[i].attenuation.constant
        );
        glUniform1f(
            mSpotLightsLocation[i].attenuation.linear, lights[i].attenuation.linear
        );
        ...
        glUniform3f(
            mSpotLightsLocation[i].direction, direction.x, direction.y, direction.z
        );
