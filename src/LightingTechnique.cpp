...
LightingTechnique::LightingTechnique(std::string vertexShaderFilename, std::string fragmentShaderFilename)
...
bool LightingTechnique::init() {
    ...
    if (!addShader(GL_VERTEX_SHADER, vertexShaderCode))
        return false;

    std::string fragmentShaderCode = readFile(mFragmentShaderFilename);
    if (!addShader(GL_FRAGMENT_SHADER, fragmentShaderCode))
        return false;
    ...
    for (auto i = 0; i < mPointLightsLocation.size(); i++) {
        ...
        mPointLightsLocation[i].ambientIntensity = getUniformLocation(name.str());

        name = boost::format("gPointLights[%1%].Base.DiffuseIntensity") % i;
        mPointLightsLocation[i].diffuseIntensity = getUniformLocation(name.str());
        ...
        mPointLightsLocation[i].attenuation.constant = getUniformLocation(name.str());

        name = boost::format("gPointLights[%1%].Attenuation.Linear") % i;
        mPointLightsLocation[i].attenuation.linear = getUniformLocation(name.str());

        name = boost::format("gPointLights[%1%].Attenuation.Exp") % i;
        mPointLightsLocation[i].attenuation.exp = getUniformLocation(name.str());
        ...
        if (mPointLightsLocation[i].color == INVALID_UNIFORM_LOCATION ||
            ...
            mPointLightsLocation[i].attenuation.constant == INVALID_UNIFORM_LOCATION ||
            mPointLightsLocation[i].attenuation.linear == INVALID_UNIFORM_LOCATION ||
    ...
    for (auto i = 0; i < mSpotLightsLocation.size(); i++) {
        ...
        mSpotLightsLocation[i].ambientIntensity = getUniformLocation(name.str());

        name = boost::format("gSpotLights[%1%].Base.Base.DiffuseIntensity") % i;
        mSpotLightsLocation[i].diffuseIntensity = getUniformLocation(name.str());
        ...
        mSpotLightsLocation[i].attenuation.constant = getUniformLocation(name.str());

        name = boost::format("gSpotLights[%1%].Base.Attenuation.Linear") % i;
        mSpotLightsLocation[i].attenuation.linear = getUniformLocation(name.str());
        ...
        if (mSpotLightsLocation[i].color == INVALID_UNIFORM_LOCATION ||
            ...
            mSpotLightsLocation[i].attenuation.constant == INVALID_UNIFORM_LOCATION ||
            mSpotLightsLocation[i].attenuation.linear == INVALID_UNIFORM_LOCATION ||
            mSpotLightsLocation[i].attenuation.exp == INVALID_UNIFORM_LOCATION ||
...
void LightingTechnique::setPointLights(const std::vector<PointLight> &lights) const {
    assert(lights.size() <= MAX_POINT_LIGHTS);

    glUniform1i(mNumPointLightsLocation, static_cast<GLint>(lights.size()));

    for (auto i = 0; i < lights.size(); i++) {
        glUniform3f(mPointLightsLocation[i].color, lights[i].color.x, lights[i].color.y, lights[i].color.z);
        glUniform1f(mPointLightsLocation[i].ambientIntensity, lights[i].ambientIntensity);
        glUniform1f(mPointLightsLocation[i].diffuseIntensity, lights[i].diffuseIntensity);
        glUniform3f(mPointLightsLocation[i].position, lights[i].position.x, lights[i].position.y, lights[i].position.z);
        glUniform1f(mPointLightsLocation[i].attenuation.constant, lights[i].attenuation.constant);
        glUniform1f(mPointLightsLocation[i].attenuation.linear, lights[i].attenuation.linear);
        glUniform1f(mPointLightsLocation[i].attenuation.exp, lights[i].attenuation.exp);
    }
}

void LightingTechnique::setSpotLights(const std::vector<SpotLight> &lights) const {
    assert(lights.size() < MAX_SPOT_LIGHTS);

    glUniform1i(mNumSpotLightsLocation, static_cast<GLint>(lights.size()));

    for (auto i = 0; i < lights.size(); i++) {
        glUniform3f(mSpotLightsLocation[i].color, lights[i].color.x, lights[i].color.y, lights[i].color.z);
        glUniform1f(mSpotLightsLocation[i].ambientIntensity, lights[i].ambientIntensity);
        glUniform1f(mSpotLightsLocation[i].diffuseIntensity, lights[i].diffuseIntensity);
        glUniform3f(mSpotLightsLocation[i].position, lights[i].position.x, lights[i].position.y, lights[i].position.z);
        glUniform1f(mSpotLightsLocation[i].attenuation.constant, lights[i].attenuation.constant);
        glUniform1f(mSpotLightsLocation[i].attenuation.linear, lights[i].attenuation.linear);
        glUniform1f(mSpotLightsLocation[i].attenuation.exp, lights[i].attenuation.exp);
        ...
        glUniform1f(mSpotLightsLocation[i].cutoff, glm::radians(lights[i].cutoff));
    }
}
